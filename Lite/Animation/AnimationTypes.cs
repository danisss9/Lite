...
public sealed class ActiveAnimation
{
    ...
    public long StartTimeMs { get; }
    public bool IsComplete { get; set; }
    /// <summary>When true, the animation is paused — elapsed time freezes.</summary>
    public bool IsPaused { get; set; }
    /// <summary>Accumulated elapsed ms before the current pause.</summary>
    public long PausedElapsed { get; set; }
    /// <summary>Tick when the animation was last resumed (or started).</summary>
    public long ResumeTimeMs { get; set; }
    /// <summary>True once the first meaningful frame has been processed (for animationstart).</summary>
    public bool HasStarted { get; set; }
    /// <summary>Tracks the last completed iteration index (for animationiteration).</summary>
    public int LastIteration { get; set; } = -1;
    ...
    public (float Offset, bool Done) GetOffset(long nowMs)
    {
        float elapsed;
        if (IsPaused)
            elapsed = PausedElapsed / 1000f - Delay;
        else
            elapsed = (PausedElapsed + (nowMs - ResumeTimeMs)) / 1000f - Delay;
