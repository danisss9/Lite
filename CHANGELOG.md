...
- Report, rather than enforce, `es2020ProfileReady` in compatibility CI. Readiness additionally requires the unfinished normative and edition review (2,115 unreviewed sections, 5,099 unresolved test classifications, eight unreviewed host obligations), which execution cannot supply. `scripts/run-es2020.py` still runs the readiness check and records its verdict, but its exit status now covers execution only. NuGet release validation continues to enforce readiness.
