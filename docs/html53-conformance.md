...
- Details attribute changes coalesce toggle notifications at the latest queued task position. Picture source selection preserves the author's `src` attribute; URL reflection and resource resolution use the owning document's base.
...
| 5. Forms and interaction | Complete control states, validity rules, ownership, successful controls, submission encodings, focus and keyboard behavior, activation, initial parser-created details notifications, modal dialogs, and shared host/headless input. |
